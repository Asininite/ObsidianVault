...
## My Explanation
- Basically we traverse the array from both sides, left and right using a pre var and suff var
- Keep multiplying the values into pre and suff using `pre * nums[i]` and `suff * nums[n - 1 - i]` , then check the max for both in the ans var;
- Essentially as the i pointer iterates through the array the subarrays keep getting bigger and we are checking the subarray from both sides.
- If there is a 0 in the array, the pre and suff will become 0. Just after the 0, we can start a new subarray and a new product, so after encountering the 0 we will set the pre and suff to 1. This is done by checking the pre and suff initially in the loop whether they are 0. 
### Solution:
```cpp
#include <bits/stdc++.h>
using namespace std;
