...
|**git add .**|Stages all changed files in the current directory and subdirectories for the next commit|git add .|
...
|**git commit**|Allows you to take staged snapshots of changes and commit them to the project and commit them to the project|git commit -m "Your commit message here"|
|**git config --global user.email**|Example 1: Sets a global email configuration for Git  <br>  <br>Example 2: Sets a global username configuration for Git|Example 1:  <br><br>git config --global user.email "your.email@example.com"<br><br>  <br>  <br>Example 2:  <br><br>git config --global user.name "Your Name"|
...
|**git fetch**|Downloads changes from a remote repository to your local repository without merging them|git fetch <options> <remote name> <branch name>|
...
|**git init**|Used to create a new local Git repository in the specified directory|git init <directory>|
...
|**git merge**|Used to merge changes from another branch into the current (active) branch|git merge feature_branch|
...
|**git request-pull**|Example 1: Creates a summary of changes for your upstream to pull  <br>  <br>Example 2: Generates a summary of pending changes for an email request|Example 1:  <br><br>git request-pull origin/main your-branch<br><br>  <br>  <br>Example 2:  <br><br>git request-pull <base> <head> <repository>|
...
|**git send-email**|Example 1: Sends your email submission without corruption by your MUA  <br>  <br>Example 2: Sends a collection of patches as emails|Example 1:  <br><br>git send-email --to=recipient@example.com<br>path/to/patchfile.patch<br><br>  <br>  <br>Example 2:  <br><br>git send-email --to recipient@example.com<br>patches/*.patch|
