...
**Pre Requisite to this problem : [Power Set](https://takeuforward.org/plus/dsa/recursion/implementation-problems/power-set)**
...
class Solution {
  public:
    void helper(int index, vector<int> &nums, int sum, vector<int> &ans){
        if(index == nums.size()){
            ans.push_back(sum);
            return;
        }

        helper(index + 1, nums, sum, ans);
        // sum += nums[index];
        helper(index + 1, nums, sum + nums[index], ans);
    }
