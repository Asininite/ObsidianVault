...
![[Pasted image 20251112200952.png]]
![[Pasted image 20251112201002.png]]

## Definition
The **Minimax algorithm** is a decision-making algorithm used in **adversarial search**.

- **Adversarial Search:** These are problems where you are in a "game" against an opponent who has conflicting goals and is also trying to win.
    
- **Use Case:** Minimax is designed for **two-player, zero-sum games** (like Tic-Tac-Toe, Chess, Checkers) where a win for one player is a loss for the other.
    
- **The Goal:** It helps a player (called **MAX**) find the _optimal_ move, assuming the opponent (called **MIN**) is also playing optimally to defeat them.
...
To understand Minimax, you need to know these terms:

1. **Game Tree:** The game is represented as a tree where nodes are game states and edges are moves.
    
2. **MAX Player:** Our player. MAX's goal is to _maximize_ the final score.
    
3. **MIN Player:** The opponent. MIN's goal is to _minimize_ the final score.
    
4. **Terminal Nodes (Leaves):** These are the end-of-game states (e.g., Win, Loss, Draw).
    
5. **Utility Function:** A function that assigns a numerical score to each terminal state.
    
    - Example: For MAX, a Win = +10, Loss = -10, Draw = 0.
